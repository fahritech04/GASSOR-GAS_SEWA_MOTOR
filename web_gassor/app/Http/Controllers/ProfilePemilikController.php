...
class ProfilePemilikController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        // Cek approval admin
        if (!$user->is_approved) {
            return redirect()->route('editprofile.pemilik')->with('error', 'Data Anda belum diverifikasi oleh admin. Silakan tunggu persetujuan admin sebelum dapat mengubah profil.');
        }
        ...
        $usernameExists = User::where('username', $validated['username'])
            ...
            ->exists();
        if ($usernameExists) {
            return redirect()->route('editprofile.pemilik')->with('error', 'Username sudah digunakan oleh pengguna lain.');
        }
        // Phone Unik
        $phoneExists = User::where('phone', $validated['phone'])
            ...
            ->exists();
        if ($phoneExists) {
            return redirect()->route('editprofile.pemilik')->with('error', 'Nomor telepon sudah digunakan oleh pengguna lain.');
        }
